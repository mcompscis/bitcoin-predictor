...
def fetch_data(symbol, time_granularity, start, end, iteration):
    ...
    #     "1day": 86400,
    # }
    url = f"https://api.pro.coinbase.com/products/{symbol}/candles?granularity={time_granularity}&start={start}&end={end}"
    ...
    if (
        ...
        if data is None:
            ...
            data.to_csv(
                f"Coinbase_{pair_split[0] + pair_split[1]}_iter{iteration}_data.csv",
                index=False,
            )
    else:
        print(start)
...
if __name__ == "__main__":
    ...
    for i, interval in enumerate(time_intervals):
        fetch_data(pair, time_interval_mapping[time], interval[0], interval[1], i)
    path = "."
    combine_csvs(path)
