...
*/
class ExtractTrainTestDataV2Conf(args: Seq[String]) extends ScallopConf(args) {
  mainOptions = Seq(blockpath)
  val blockpath = opt[String](descr = "block file", required = false)
  verify()
}

object ExtractTrainTestDataV2 {
    ...
    def main(argv: Array[String]): Unit = {
        val args = new ExtractTrainTestDataV2Conf(argv)
        ...
        // df.repartition(4)
        // df = df.withColumn("changeInBtcAmtBtwCurrAnd1BlockAgo", col("totalBtcAmountReceivedInBlock") - col("totalBtcAmountReceived1BlockAgo"))
        // df = df.withColumn("changeInBtcAmtBtw1And2BlocksAgo", col("totalBtcAmountReceived1BlockAgo") - col("totalBtcAmountReceived2BlocksAgo"))
        // df = df.withColumn("changeInBtcAmtBtw2And3BlocksAgo", col("totalBtcAmountReceived2BlocksAgo") - col("totalBtcAmountReceived3BlocksAgo"))
        // df = df.withColumn("changeInBtcAmtBtw3And4BlocksAgo", col("totalBtcAmountReceived3BlocksAgo") - col("totalBtcAmountReceived4BlocksAgo"))
        // df = df.withColumn("changeInNumTxnsBtwCurrAnd1BlockAgo", col("numTxnsInBlock") - col("numTxns1BlockAgo"))
